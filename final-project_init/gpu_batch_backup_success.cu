...
static int hostBegin[MAX_NUM_NODE], hostEnd[MAX_NUM_NODE];
...
void matmul(Tensor *input1, Tensor *input2, Tensor *output) // 단어개수 : 256개, M : 1024,  K: 512, 1024, N : 64(batchsize)
{
  ...
  for (int i = 0; i < num_devices; i++) {
    ...
    CUDA_CALL(cudaMalloc(&a_d[i], (Nend[i] - Nbegin[i]) * K_ * sizeof(float)));
    CUDA_CALL(cudaMalloc(&b_d[i], K_ * N_ * sizeof(float)));
    CUDA_CALL(cudaMalloc(&c_d[i], (Nend[i] - Nbegin[i]) * N_ * sizeof(float)));
  } 
  ...
  for (int i = 0; i < num_devices; i++) { //a_d (global) <- input1 (cpu)
    CUDA_CALL(cudaMemcpy(a_d[i], input1->buf + Nbegin[i] * K_,
                         (Nend[i] - Nbegin[i]) * K_ * sizeof(float),
  ...
  for (int i = 0; i < num_devices; i++) {
    ...
    dim3 gridDim((N_ - 1) / TS + 1, (Nend[i] - Nbegin[i] - 1) / TS + 1, 1);
  ...
  for (int i = 0; i < num_devices; i++) {
    CUDA_CALL(cudaMemcpy(output->buf + Nbegin[i] * N_, c_d[i],
                        (Nend[i] - Nbegin[i]) * N_ * sizeof(float),
...
void namegen(int N, float *random_floats, char *output) {
  ...
  int bs = hostEnd[mpi_rank]-hostBegin[mpi_rank];

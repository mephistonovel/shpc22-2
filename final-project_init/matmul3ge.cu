...
static int cudabegin1[MAX_NUM_GPU], cudaend1[MAX_NUM_GPU];
static int cudabegin2[MAX_NUM_GPU], cudaend2[MAX_NUM_GPU];
static int cudabegin3[MAX_NUM_GPU], cudaend3[MAX_NUM_GPU];
static int mrbegin[MAX_NUM_GPU], mrend[MAX_NUM_GPU];
static int rhstart[MAX_NUM_NODE], rhend[MAX_NUM_NODE];

static float4 *a_d1[MAX_NUM_GPU];
static float4 *b_d1[MAX_NUM_GPU];
static float4 *c_d1[MAX_NUM_GPU];
static float4 *a_d2[MAX_NUM_GPU];
static float4 *b_d2[MAX_NUM_GPU];
static float4 *c_d2[MAX_NUM_GPU];
static float4 *a_d3[MAX_NUM_GPU];
static float4 *b_d3[MAX_NUM_GPU];
static float4 *c_d3[MAX_NUM_GPU];
...
void matmul1(Tensor *input1, Tensor *input2, Tensor *output) // 단어개수 : 256개, M : 1024,  K: 512, 1024, N : 64(batchsize)
{
  ...
  for (int i = 0; i < num_devices; i++) { //a_d (global) <- input1 (cpu)
    CUDA_CALL(cudaMemcpy(a_d1[i], input1->buf + cudabegin1[i] * K_,
                         (cudaend1[i] - cudabegin1[i]) * K_ * sizeof(float),
                         cudaMemcpyHostToDevice));
    CUDA_CALL(
        cudaMemcpy(b_d1[i], input2->buf, K_ * N_ * sizeof(float), cudaMemcpyHostToDevice));
  }
  ...
  for (int i = 0; i < num_devices; i++) {
    ...
    // dim3 blockDim(TS, TS, 1);
    dim3 gridDim((cudaend1[i] - cudabegin1[i] +TS- 1)/TS, (N_/width + TS/width-1)/(TS/width),1);
    dim3 blockDim(TS, TS/width, 1);

    CUDA_CALL(cudaSetDevice(i));
    // gridDim: block 개수
    // blockDim: thread 개수
    matmul_kernel<<<gridDim, blockDim>>>(a_d1[i], b_d1[i], c_d1[i], M_, N_, K_);
  }
  for (int i = 0; i < num_devices; i++) {
  ...
  for (int i = 0; i < num_devices; i++) {
    CUDA_CALL(cudaMemcpy(output->buf + cudabegin1[i] * N_, c_d1[i],
                        (cudaend1[i] - cudabegin1[i]) * N_ * sizeof(float),
...
void matmul2(Tensor *input1, Tensor *input2, Tensor *output) // 단어개수 : 256개, M : 1024,  K: 512, 1024, N : 64(batchsize)
{
  size_t M_ = input1->shape[0];
  size_t K_ = input1->shape[1];
  size_t N_ = input2->shape[1];
  ...
  for (int i = 0; i < num_devices; i++) { //a_d (global) <- input1 (cpu)
    CUDA_CALL(cudaMemcpy(a_d2[i], input1->buf + cudabegin2[i] * K_,
                         (cudaend2[i] - cudabegin2[i]) * K_ * sizeof(float),
                         cudaMemcpyHostToDevice));
    CUDA_CALL(
        cudaMemcpy(b_d2[i], input2->buf, K_ * N_ * sizeof(float), cudaMemcpyHostToDevice));
  }
  ...
  for (int i = 0; i < num_devices; i++) {
    ...
    // dim3 gridDim((N_ - 1) / TS + 1, (cudaend[i] - cudabegin[i] - 1) / TS + 1, 1);
    // dim3 blockDim(TS, TS, 1);
    dim3 gridDim((cudaend2[i] - cudabegin2[i] +TS- 1)/TS, (N_/width + TS/width-1)/(TS/width),1);
    dim3 blockDim(TS, TS/width, 1);

    CUDA_CALL(cudaSetDevice(i));
    // gridDim: block 개수
    // blockDim: thread 개수
    matmul_kernel<<<gridDim, blockDim>>>(a_d2[i], b_d2[i], c_d2[i], M_, N_, K_);
  }
  for (int i = 0; i < num_devices; i++) {
    CUDA_CALL(cudaDeviceSynchronize());
  }
  // Download C matrix from GPUs
  for (int i = 0; i < num_devices; i++) {
    CUDA_CALL(cudaMemcpy(output->buf + cudabegin2[i] * N_, c_d2[i],
                        (cudaend2[i] - cudabegin2[i]) * N_ * sizeof(float),
                        cudaMemcpyDeviceToHost));
  }
  MPI_Barrier(MPI_COMM_WORLD);
...
void matmul3(Tensor *input1, Tensor *input2, Tensor *output) // 단어개수 : 256개, M : 1024,  K: 512, 1024, N : 64(batchsize)
{
  size_t M_ = input1->shape[0];
  size_t K_ = input1->shape[1];
  size_t N_ = input2->shape[1];
  ...
  for (int i = 0; i < num_devices; i++) {
    CUDA_CALL(cudaSetDevice(i));
    CUDA_CALL(cudaMalloc(&a_d3[i], (cudaend3[i] - cudabegin3[i]) * K_ * sizeof(float)));
    CUDA_CALL(cudaMalloc(&b_d3[i], K_ * N_ * sizeof(float)));
    CUDA_CALL(cudaMalloc(&c_d3[i], (cudaend3[i] - cudabegin3[i]) * N_ * sizeof(float)));
  } 
  ...
  for (int i = 0; i < num_devices; i++) { //a_d (global) <- input1 (cpu)
    CUDA_CALL(cudaMemcpy(a_d3[i], input1->buf + cudabegin3[i] * K_,
                         (cudaend3[i] - cudabegin3[i]) * K_ * sizeof(float),
                         cudaMemcpyHostToDevice));
    CUDA_CALL(
        cudaMemcpy(b_d3[i], input2->buf, K_ * N_ * sizeof(float), cudaMemcpyHostToDevice));
  }
  ...
  for (int i = 0; i < num_devices; i++) {
    ...
    // dim3 gridDim((N_ - 1) / TS + 1, (cudaend[i] - cudabegin[i] - 1) / TS + 1, 1);
    // dim3 blockDim(TS, TS, 1);
    dim3 gridDim((cudaend3[i] - cudabegin3[i] +TS- 1)/TS, (N_/width + TS/width-1)/(TS/width),1);
    dim3 blockDim(TS, TS/width, 1);

    CUDA_CALL(cudaSetDevice(i));
    // gridDim: block 개수
    // blockDim: thread 개수
    matmul_kernel<<<gridDim, blockDim>>>(a_d3[i], b_d3[i], c_d3[i], M_, N_, K_);
  }
  for (int i = 0; i < num_devices; i++) {
    CUDA_CALL(cudaDeviceSynchronize());
  }
  // Download C matrix from GPUs
  for (int i = 0; i < num_devices; i++) {
    CUDA_CALL(cudaMemcpy(output->buf + cudabegin3[i] * N_, c_d3[i],
                        (cudaend3[i] - cudabegin3[i]) * N_ * sizeof(float),
                        cudaMemcpyDeviceToHost));
  }
  MPI_Barrier(MPI_COMM_WORLD);
...
void namegen_initialize(int N, int rng_seed, char *parameter_fname) {
  ...
  {
    CUDA_CALL(cudaSetDevice(i));
    int M1 = 1024; int K1 = 512; int N1 = bs;
    int M2 = 1024; int K2 = 1024; int N2 = bs;
    int M3 = 256; int K3 = 1024; int N3 = bs;
    cudabegin1[i] = M1/4 * i ;
    cudaend1[i] = M1/4 * (i+1);
    cudabegin2[i] = M2/4 * i ;
    cudaend2[i] = M2/4 * (i+1);
    cudabegin3[i] = M2/4 * i ;
    cudaend3[i] = M2/4 * (i+1);
    CUDA_CALL(cudaMalloc(&a_d1[i], (cudaend1[i] - cudabegin1[i]) * K1 * sizeof(float)));
    CUDA_CALL(cudaMalloc(&b_d1[i], K1 * N1 * sizeof(float)));
    CUDA_CALL(cudaMalloc(&c_d1[i], (cudaend1[i] - cudabegin1[i]) * N1 * sizeof(float)));
    CUDA_CALL(cudaMalloc(&a_d2[i], (cudaend2[i] - cudabegin2[i]) * K2 * sizeof(float)));
    CUDA_CALL(cudaMalloc(&b_d2[i], K2 * N2 * sizeof(float)));
    CUDA_CALL(cudaMalloc(&c_d2[i], (cudaend2[i] - cudabegin2[i]) * N2 * sizeof(float)));
    CUDA_CALL(cudaMalloc(&a_d3[i], (cudaend3[i] - cudabegin3[i]) * K3 * sizeof(float)));
    CUDA_CALL(cudaMalloc(&b_d3[i], K3 * N3 * sizeof(float)));
    CUDA_CALL(cudaMalloc(&c_d3[i], (cudaend3[i] - cudabegin3[i]) * N3 * sizeof(float)));
  }
...
void namegen(int N, float *random_floats, char *output) {
  ...
  for (int b_i =0;b_i<loop_num ; b_i++){
    ...
    for (int l = 0; l < MAX_LEN; l++) {
      ...
      /* First layer r */
      matmul1(W_ir0, emb_out, rtmp00); //hid*bs  W : H * E, embe_out : E*B
      matmul2(W_hr0, hidden0, rtmp01); //hid*bs  W : H * H, hidden : H*B
      ...
      /* First layer z */
      matmul1(W_iz0, emb_out, ztmp00); //hid*bs  W : H * E, embe_out : E*B
      matmul2(W_hz0, hidden0, ztmp01); //hid*bs  W : H * H, hidden : H*B
      ...
      matmul1(W_in0, emb_out, ntmp00);//win * x = hid*bs
      matrow_add(ntmp00, b_in0, ntmp01);//hid*bs
      matmul2(W_hn0, hidden0, ntmp02); //hid*bs
      ...
      /* Second layer r */
      matmul2(W_ir1, hidden0, rtmp10); //hid*bs
      matmul2(W_hr1, hidden1, rtmp11); //hid*bs
      ...
      /* Second layer z */
      matmul2(W_iz1, hidden0, ztmp10); //hid*bs
      matmul2(W_hz1, hidden1, ztmp11); //hid*bs
      ...
      matmul2(W_in1, hidden0, ntmp10); //hid*bs
      matrow_add(ntmp10, b_in1, ntmp11); //hid*bs
      matmul2(W_hn1, hidden1, ntmp12); //hid*bs
      ...
      matmul3(W_fc, hidden1, ftmp0); // (numchar, hid) * (hid,bs) = numchar*bs
...
//   for (int i = 0; i < num_devices; i++) {
//     CUDA_CALL(cudaFree(a_d1[i]));
//     CUDA_CALL(cudaFree(b_d1[i]));
//     CUDA_CALL(cudaFree(c_d1[i]));
//     CUDA_CALL(cudaFree(a_d2[i]));
//     CUDA_CALL(cudaFree(b_d2[i]));
//     CUDA_CALL(cudaFree(c_d2[i]));
//     CUDA_CALL(cudaFree(a_d3[i]));
//     CUDA_CALL(cudaFree(b_d3[i]));
//     CUDA_CALL(cudaFree(c_d3[i]));
// } 
}